...
class iSplitter:
    ...
    def CreateImageFromGroup(self, groupid, max=False):
        ...
        if max != False and len(group) < max:
            # print("Returning false... Len of group {0}".format(len(group)))
            return False
    ...
    def imagifyGroups(self, max=50, endat=-1):
        imageSet, endcheck = [], 0
        cimage = np.array(self.ContainedImage)
        for gid, group in enumerate(self.Groups):
            if endcheck == endat: break
            img = self.CreateImageFromGroup(gid, max=max)
            if img == False: continue
            x, y = self.Groups[gid][0]
            color = cimage[y][x]
            imageSet.append({
                "rawimage": img,
                "color": color
